...
struct UpdateCacheUniforms {
    ...
    token_dim: u32,       // Dimension of each token's K or V vector
    num_heads: u32,       // Number of attention heads
    _pad: u32,            // Padding for alignment
}

@group(0) @binding(0) var<storage, read> new_kv: array<f32>;          // New K or V for current token (num_heads * head_dim)
@group(0) @binding(1) var<storage, read_write> cache: array<f32>;     // KV cache buffer (max_seq_len * num_heads * head_dim)
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // Total elements to copy (num_heads * head_dim for one token)
    ...
    // Cache layout: [max_seq_len][num_heads][head_dim]
