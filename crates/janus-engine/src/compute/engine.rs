            ...
            match tensor.dtype {
                ...
                TensorDType::Q4_K => {
                    if Self::is_embedding_tensor_name(&name) {
                        ...
                        tracing::debug!(
                            "Allocated tensor '{}': {} bytes (Q4_K quantized)",
                            name,
                            size_bytes
                        );
                ...
                TensorDType::Q5_K => {
                    if Self::is_embedding_tensor_name(&name) {
                        ...
                        tracing::debug!(
                            "Allocated tensor '{}': {} bytes (Q5_K quantized)",
                            name,
                            size_bytes
                        );
                ...
                TensorDType::Q8_0 => {
                    if Self::is_embedding_tensor_name(&name) {
                        ...
                        tracing::debug!(
                            "Allocated tensor '{}': {} bytes (Q8_0 quantized)",
                            name,
                            size_bytes
                        );
                ...
                TensorDType::Q6_K => {
                    println!("⚙️ [Loader] Auto-detect: Q6_K tensor found. Dequantizing to packed F16 fallback on CPU...");
                    let num_elements: usize = tensor.shape.iter().product();
                    let f32_data = Self::q6_k_to_f32(tensor.data, num_elements)?;
                    let f16_data: Vec<f16> = f32_data.into_iter().map(f16::from_f32).collect();
                    let packed_data = Self::pack_f16_to_u32(&f16_data);
                    let packed_bytes = bytemuck::cast_slice(&packed_data);
                    ...
                    tracing::debug!(
                        "Allocated tensor '{}': {} bytes (Q6_K -> packed FP16 fallback, {} elements)",
                        name,
                        packed_bytes.len(),
                        num_elements
                    );
