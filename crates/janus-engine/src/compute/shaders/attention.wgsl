...
// - Keys: [seq_len][num_kv_heads][head_dim] - all cached keys
// - Values: [seq_len][num_kv_heads][head_dim] - all cached values
...
@group(0) @binding(1) var<storage, read> keys: array<f32>;       // [seq_len * num_kv_heads * head_dim]
...
fn compute_qk_scores(
    ...
    for (var d = 0u; d < head_dim; d++) {
        ...
        let k_idx = key_pos * params.num_kv_heads * head_dim + kv_head_idx * head_dim + d;
...
@group(0) @binding(1) var<storage, read> values: array<f32>;          // [seq_len * num_kv_heads * head_dim]
