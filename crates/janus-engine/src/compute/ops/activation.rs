...
///
/// # DEPRECATED
/// This is the old async version. Use `rmsnorm_static()` for the new static computation graph pattern.
...
/// Apply Root Mean Square Normalization (RMSNorm) - Static computation graph version
///
/// Formula: output[i] = (input[i] / sqrt(mean(input^2) + epsilon)) * gamma[i]
///
/// # Arguments
...
/// Apply SiLU (Swish) activation function - Static computation graph version
///
/// f(x) = x * sigmoid(x)
///
/// # Arguments
